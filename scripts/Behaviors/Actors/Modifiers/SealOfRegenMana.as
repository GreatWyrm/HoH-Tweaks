namespace Modifiers
{
	class SealOfRegenMana : Modifier
	{
		vec2 m_regen;
		bool affectHealth;
		bool affectMana;
	
		SealOfRegenMana(UnitPtr unit, SValue& params)
		{		
			m_regen = vec2(
		...
		vec2 RegenAdd(PlayerBase@ player) override {
			auto regen = vec2(0.0f, 0.0f) + m_regen * clamp(1.0f - player.m_record.mana, 0.0f, 1.0f) * 10.0f;
			if(!affectHealth) {
				regen.x = 0.0f;
			}
			if(!affectMana) {
